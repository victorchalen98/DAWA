...
class LoginComponent:
    ...
    @staticmethod
    def Login(p_usuario, p_nombre):
        try:
            result = False
            data = None
            message = None
            sql = "SELECT count(*) as valor FROM dawa.USUARIO WHERE usuario_login = %s AND usuario_nombre = %s"
            record = (p_usuario, p_nombre)
            ...
            if resul_login['result']:
                if resul_login['data']['valor'] > 0:
                    ...
                    data = JwtComponent.TokenGenerate(p_usuario)
                    print(data)
                else:
